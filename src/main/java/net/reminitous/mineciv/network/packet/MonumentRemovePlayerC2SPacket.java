...
public class MonumentRemovePlayerC2SPacket {
    private final BlockPos monumentPos;
    private final String playerName;

    public MonumentRemovePlayerC2SPacket(BlockPos monumentPos, String playerName) {
        this.monumentPos = monumentPos;
        this.playerName = playerName;
    }

    public MonumentRemovePlayerC2SPacket(FriendlyByteBuf buf) {
    ...
    public boolean handle(Supplier<NetworkEvent.Context> supplier) {
        ...
        context.enqueueWork(() -> {
            ...
            boolean success = ChunkClaimManager.removePlayerAccess(sender.level(), chunkX, chunkZ, targetUUID);

            if (success) {
                sender.sendSystemMessage(Component.literal("Removed " + playerName + " from allowed players"));
                ...
                if (targetPlayer != null) {
                    targetPlayer.sendSystemMessage(Component.literal(
                            sender.getName().getString() + " has revoked your access to their chunk at ["
                                    + chunkX + ", " + chunkZ + "]"));
                }
            } else {
                sender.sendSystemMessage(Component.literal("Failed to remove " + playerName));
            }
        });
        return true;
    }
